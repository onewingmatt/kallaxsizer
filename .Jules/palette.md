...
## 2025-05-15 - [Monolithic UI: Input ID Consistency and Enter Support]
**Learning:** In a monolithic `index.html`, inconsistent ID references (e.g., `bggUser` vs `bggUsername`) can cause silent failures in async operations like caching. Implementing standard "Enter" key support for primary inputs is a high-impact micro-UX win that significantly improves efficiency for power users.
**Action:** Always verify DOM IDs when implementing new listeners in large files. Standardize "Enter" triggers for both main search inputs and modal data-entry fields to improve user flow.
